...
- `GET /v1/events` returns `{ events: EngineEvent[] }` as a snapshot scaffold with `after` and `limit` query support.
...
Known limitation: this is a snapshot scaffold, not a long-lived server-sent event stream. Real libtorrent alerts still need throttling and UI subscription behavior.
