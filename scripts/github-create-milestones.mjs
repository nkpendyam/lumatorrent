...
for(const m of milestones){
  ...
  const argv=['api',`repos/${owner}/${repo}/milestones`,'-X','POST','-f',`title=${body.title}`,'-f',`description=${body.description}`];
  console.log(`$ gh ${argv.join(' ')}`);
  if(execute){const res=spawnSync('gh',argv,{stdio:'inherit'}); if(res.status!==0) console.warn(`Milestone may already exist: ${m.title}`)}
}
