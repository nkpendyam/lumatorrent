...
function have(cmd) {
  const result = spawnSync(process.platform === 'win32' ? 'where' : 'which', [cmd], { encoding: 'utf8' });
...
if (have('gh')) {
  ...
  console.log(auth.status === 0 ? 'OK gh authenticated' : 'MISSING gh authentication: run gh auth login');
